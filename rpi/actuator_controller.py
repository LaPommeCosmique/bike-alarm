...
def on_tamper_message(channel, method, properties, msg):
    try:
        ...
        if msg is not None:
            if msg["tamper"]:
                mqttc.publish(buzzer_topic, json.dumps({'buzzer':True}))
            else:
                mqttc.publish(buzzer_topic, json.dumps({'buzzer':False}))
    except Exception as e:
        logging.warning(f'{e}')
...
amqp_client.subscribe(amqp_channel, lock_status_queue_rabbit, lock_status_r_key_rabbit)
amqp_client.subscribe(amqp_channel, tamper_status_queue_rabbit, tamper_status_r_key_rabbit)
amqp_client.subscribe(amqp_channel, compromised_status_queue_rabbit, compromised_status_r_key_rabbit)

## MQTT CONNECTION PARAMETERS
ip_mosquitto = "localhost"
lock_status_led_topic = "lock_status_led_topic"
buzzer_topic = "buzzer_topic_topic"
